	...
	)
{
	...
	(this value is not needed for CK48 configuration) */
	pplQ = ((RCC->PLLSAICFGR & RCC_PLLSAICFGR_PLLSAIQ) >> RCC_PLLSAICFGR_PLLSAIQ_Pos);

	/* Configure the PLLSAI division factors */
	/* PLLSAI_VCO = f(VCO clock) = f(PLLSAI clock input) x (PLLI2SN/PLLM) */
	/* 48CLK = f(PLLSAI clock output) = f(VCO clock) / PLLSAIP */
	RCC->PLLSAICFGR = (nPLLSAIN<<RCC_PLLSAICFGR_PLLSAIN_Pos)
                      | (nPLLSAIP<<RCC_PLLSAICFGR_PLLSAIP_Pos)
                      | (pplQ<<RCC_PLLSAICFGR_PLLSAIQ_Pos);
